                                ...
                                <table class="table table-hover text-nowrap">
                                    ...
                                    @foreach($posts as $post)
                                        <tr>
                                            <td>{{ $post->id }}</td>
                                            <td>{{ $post->title }}</td>
                                            <td><a class="text-green" href="{{ route('admin.post.edit', $post->id)  }}"><i class="fa-solid fa-pencil"></i></a></td>
                                            <td>
                                                <form class="w-100" method="post" action="{{ route('admin.post.delete', $post->id) }}">
                                                    @csrf
                                                    @method('DELETE')
